...
class BrainTumorEnv(gymnasium.Env):
    def __init__(self, images, target_boxes):
        ...
        self.observation_space = spaces.Dict({
            "image": spaces.Box(low=0, high=1, shape=(224, 224, 3), dtype=np.float32),
...
class DQNAgent:
    ...
    def act(self, state, training=True):
        ...
        # OTTIMIZZAZIONE: Usato model() invece di predict()
    ...
    def replay(self, batch_size):
        ...
        states_img = np.array([s['image'] for s, a, r, ns, d in minibatch], dtype=np.float32)
        states_box = np.array([s['box'] for s, a, r, ns, d in minibatch], dtype=np.float32)
        next_states_img = np.array([ns['image'] for s, a, r, ns, d in minibatch], dtype=np.float32)
        next_states_box = np.array([ns['box'] for s, a, r, ns, d in minibatch], dtype=np.float32)
        ...
        # OTTIMIZZAZIONE: Uso di GradientTape invece di model.fit
        with tf.GradientTape() as tape:
            ...
            # Creiamo una maschera per aggiornare solo l'azione presa
            masks = tf.one_hot(actions, self.action_size)
            
            # Calcoliamo la loss (MSE) solo per le azioni scelte
...
def visualize_progress(env, agent, episode):
    ...
    for i in range(num_samples):
        # Scegliamo un'immagine casuale per il test
        idx = random.randint(0, len(env.images) - 1)
        
        # Resettiamo l'ambiente e forziamo l'indice dell'immagine scelta
        state, _ = env.reset()
        env.current_img_idx = idx
        state['image'] = env.images[idx] # Sincronizziamo l'osservazione con l'immagine scelta
        
        # Simuliamo l'agente per un numero limitato di passi usando l'ambiente
        for _ in range(50):
            action = agent.act(state, training=False)
            # Usiamo env.step invece di riscrivere la logica dei movimenti
            next_state, _, terminated, truncated, _ = env.step(action)
            state = next_state
            if terminated or truncated:
                break
        
        # Recuperiamo i dati finali per il plotting
        img = env.images[idx]
        ...
        # Disegniamo la box reale (rossa) e quella dell'agente (verde)
        rect_real = patches.Rectangle((target_box[0]*224, target_box[1]*224), 
                                      target_box[2]*224, target_box[3]*224, 
                                      linewidth=2, edgecolor='r', facecolor='none', label='Reale')
        rect_agent = patches.Rectangle((current_box[0]*224, current_box[1]*224), 
                                       current_box[2]*224, current_box[3]*224, 
