...
def compute_action_steps(W: int, H: int, frac: float = 0.045, min_step: float = 6.0):
    """
    frac controlla la dimensione del passo come frazione delle dimensioni immagine.
    Viene schedulato durante il training (grosso all'inizio, fine alla fine) tramite
    StepSizeScheduleCallback, cosi' l'agente puo' sia convergere velocemente sia
    rifinire con precisione nelle fasi avanzate.
    """
    big_step = max(min_step, frac * ((W + H) / 2.0))
    big_step_w = max(min_step, frac * W)
    big_step_h = max(min_step, frac * H)
...
class StepSizeScheduleCallback(BaseCallback):
    ...
    passi grossi all'inizio (convergenza rapida verso la zona giusta), passi fini
    verso la fine (rifinitura di precisione). Elimina il limite strutturale dato
    da uno step fisso, che impediva di raggiungere IoU alte.
    """
    def __init__(self, curriculum_steps, initial_frac=0.045, final_frac=0.012, verbose=0):
        super().__init__(verbose)
        self.curriculum_steps = curriculum_steps
...
class BrainTumorRL_Env(gym.Env):
    ...
    def __init__(self, pytorch_dataset, max_steps=MAX_STEPS_PER_EPISODE, min_steps_before_stop=0):
        ...
        self.step_frac = 0.045
        self.big_step, self.big_step_w, self.big_step_h = compute_action_steps(self.W, self.H, self.step_frac)
    ...
    def reset(self, seed=None, options=None):
        ...
        # Inizializzazione 100% casuale, Curriculum sulla posizione rimosso
        # (il curriculum ora agisce su STOP e su step_frac, non sulla box iniziale)
    ...
    def step(self, action):
        ...
        # ─── AZIONI DI MOVIMENTO/RESIZE (0-7), come originale ───
        bs = self.big_step
        bs_w = self.big_step_w
        bs_h = self.big_step_h
