...
class BatchedActiveLocalizationEnv:
 ...
 # delta espliciti sui 4 estremi (x1, y1, x2, y2)
    ...
    def compute_oracle_actions(self, lookahead=ORACLE_LOOKAHEAD_STEPS):
        """Oracolo con lookahead a più passi (default: ORACLE_LOOKAHEAD_STEPS).

        Un oracolo greedy a un solo passo non può mai "vedere" il valore di una
        mossa che nell'immediato peggiora il CIoU (es. un expand fatto per
        riguadagnare margine di manovra dopo uno shrink prematuro) ma che apre la
        strada a un miglioramento maggiore uno o due step dopo. Con lookahead=1
        l'oracolo non dimostrerà mai queste sequenze di recupero, e quindi
        l'imitazione (DQfD margin loss) non le impara. Con lookahead>=2 l'oracolo
        valuta esplicitamente "se faccio questa mossa ora, qual è il miglior CIoU
        raggiungibile entro le prossime N mosse", scegliendo la prima mossa della
        sequenza migliore.
        """
        shrink_allowed = self.get_shrink_allowed_mask()  # [num_envs], stato REALE corrente

        def masked_ciou(boxes):
            return compute_ciou_tensor(boxes, self.gt_boxes)

        def best_value(boxes, depth):
            """Miglior CIoU raggiungibile da 'boxes' in al più 'depth' mosse aggiuntive."""
            if depth <= 0:
                return masked_ciou(boxes)
            candidates = self._simulate_move_boxes(boxes)
            vals = torch.stack([best_value(cb, depth - 1) for cb in candidates], dim=1)  # [N, 8]
            # La fase (shrink permesso o no) è quella dello stato REALE corrente:
            # è un'approssimazione (nel lookahead futuro la fase potrebbe sbloccarsi),
            # ma evita che l'oracolo pianifichi sequenze che l'agente non potrebbe
            # eseguire nell'immediato per via della stessa maschera di fase.
            vals[~shrink_allowed, 4] = -1e9
            vals[~shrink_allowed, 5] = -1e9
            return vals.max(dim=1)[0]

        candidates = self._simulate_move_boxes(self.boxes)
        values = torch.stack([best_value(cb, max(lookahead - 1, 0)) for cb in candidates], dim=1)  # [N, 8]

        # Nota: non mascheriamo le mosse "clippate" dal bordo (quelle il cui effetto
        # viene troncato dal clamp sui limiti dell'immagine): l'oracolo deve poter
        # scegliere comunque un'azione che si scontra col bordo se resta la migliore.
        values[~shrink_allowed, 4] = -1e9
        values[~shrink_allowed, 5] = -1e9
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backbone-freeze", type=str, default="all", choices=["all"],
                        help="Presente solo per compatibilità con la documentazione/comandi storici: "
                             "in questa versione ridotta il backbone (ResNet18 + spatial_pool) e' "
                             "SEMPRE congelato (solo EmbeddingReplayBuffer, niente fine-tuning "
                             "durante l'RL), quindi l'unico valore accettato e' 'all'.")
