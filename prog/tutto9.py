...
class BatchedActiveLocalizationEnv:
    ...
    def step(self, actions):
        ...
        rewards[regressed_mask] -= 0.2 * (self.best_ious[regressed_mask] - new_ious[regressed_mask])
    ...
    def compute_oracle_actions(self):
        ...
        ious = torch.stack([compute_giou_tensor(cb, self.gt_boxes) for cb in candidates], dim=1)
        best_move_action = ious.argmax(dim=1)
...
def train(args, device, train_ds, val_ds):
    ...
    if args.model is not None:
        ...
        print(f"  [✓] Ripresa da epoca {start_epoch}, global_step={global_step}, best_val_iou={best_val_iou:.4f}")
        if start_epoch >= args.n_epochs:
            print(f"[INFO] Epoca di partenza già >= --n-epochs: fine.")
            writer.close()
