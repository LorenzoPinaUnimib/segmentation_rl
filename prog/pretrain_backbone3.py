...
# Importa DIRETTAMENTE da tutto10.py il modulo di pooling spaziale e le costanti
# rilevanti, invece di duplicarne la definizione: garantisce che l'architettura
# pre-addestrata qui sia BIT-A-BIT identica a quella usata da PolicyNetwork in
# tutto10.py (stessa classe, stessi nomi di parametri), quindi il caricamento dei
# pesi con --pretrained-backbone funziona senza sorprese.
# NOTA: prima importava da "tutto3" — l'ho aggiornato a "tutto10" perché è la
# versione dello script RL che stiamo usando/aggiornando ora. Se nel tuo progetto
# usi ancora effettivamente tutto3.py per l'RL, cambia questa riga di conseguenza:
# l'importante è che punti ESATTAMENTE al file che poi lanci per il training RL,
# altrimenti SpatialAttentionPool/EMBED_DIM potrebbero non coincidere bit-a-bit.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from tutto14 import SpatialAttentionPool, EMBED_DIM, WARP_SIZE  # noqa: E402


# ─────────────────────────────────────────────────────────────────────────────
# DECODER: blocco di upsampling + fusione con la skip connection dell'encoder
# ─────────────────────────────────────────────────────────────────────────────
class UpBlock(nn.Module):
    """Upsample x2 + concatenazione con la feature map dell'encoder allo stesso
    stadio (skip connection, stile U-Net) + due conv 3x3 di fusione. Rispetto a
    un decoder FCN puro (solo upsampling in sequenza dal fondo), questo lascia
    al decoder l'accesso diretto al dettaglio spaziale fine che l'encoder aveva
    prima di essere sotto-campionato — fondamentale per bordi netti su strutture
    piccole come un tumore in MRI, e per spingere l'encoder a PRESERVARE quel
    dettaglio invece di poterlo scartare senza conseguenze."""
...
# ─────────────────────────────────────────────────────────────────────────────
# MODELLO: encoder ResNet18 + spatial_pool (identico a tutto10.py) + decoder
# U-Net (con skip connections) per segmentazione + testa ausiliaria di
# regressione del bounding box
# ─────────────────────────────────────────────────────────────────────────────
class ResNet18SegModel(nn.Module):
    def __init__(self, init_weights="imagenet"):
        """init_weights:
        - "imagenet": pesi torchvision standard (default, comportamento originale)
        - "none"/None: nessuna inizializzazione (random) — usato in run_test(),
          dove i pesi veri arrivano comunque dal checkpoint caricato dopo
        - percorso file (.pt/.pth): state_dict custom da caricare nel backbone
          (es. pesi pre-addestrati su immagini radiologiche tipo RadImageNet,
          in genere un punto di partenza migliore dell'ImageNet naturale per MRI:
          chiude parte del domain gap prima ancora di iniziare il fine-tuning).
        """
        super().__init__()
        if init_weights in (None, "none", "None"):
            ...
            state = state.get("state_dict", state.get("backbone_state_dict", state)) if isinstance(state, dict) else state
            cleaned = {}
            ...
            print(f"[INFO] Pesi custom caricati da '{init_weights}' "
                  f"(missing={len(missing)}, unexpected={len(unexpected)})")

        backbone.fc = nn.Identity()
        self.backbone = backbone  # <-- struttura IDENTICA a PolicyNetwork.backbone

        # Stesso modulo usato da PolicyNetwork in tutto10.py: conv leggero +
        ...
        # tutto10.py (che ha la sua testa Q) e quindi NON viene salvata nel file
        ...
        # Decoder U-Net: 5 upsampling x2 = x32 totale, con skip connection dai
        # 4 stadi dell'encoder (stem, layer1, layer2, layer3). Canali ResNet18:
        # stem/layer1=64, layer2=128, layer3=256, layer4=512.
        self.up1 = UpBlock(in_ch=512, skip_ch=256, out_ch=256)  # 7x7   -> 14x14, skip layer3
        self.up2 = UpBlock(in_ch=256, skip_ch=128, out_ch=128)  # 14x14 -> 28x28, skip layer2
        self.up3 = UpBlock(in_ch=128, skip_ch=64,  out_ch=64)   # 28x28 -> 56x56, skip layer1
        self.up4 = UpBlock(in_ch=64,  skip_ch=64,  out_ch=32)   # 56x56 -> 112x112, skip stem
        self.up5 = UpBlock(in_ch=32,  skip_ch=0,   out_ch=16)   # 112x112 -> 224x224, nessuna skip
        self.seg_head = nn.Conv2d(16, 1, kernel_size=1)

    def forward_backbone_features(self, x):
        """Ritorna le feature map a TUTTI gli stadi dell'encoder (non solo
        l'ultima), necessarie per le skip connection del decoder U-Net."""
        b = self.backbone
        x0 = b.conv1(x)
        x0 = b.bn1(x0)
        x0 = b.relu(x0)          # [B, 64, 112, 112]  (stem, pre-maxpool)
        x1 = b.maxpool(x0)
        x1 = b.layer1(x1)        # [B, 64, 56, 56]
        x2 = b.layer2(x1)        # [B, 128, 28, 28]
        x3 = b.layer3(x2)        # [B, 256, 14, 14]
        x4 = b.layer4(x3)        # [B, 512, 7, 7]
        return x0, x1, x2, x3, x4

    def forward_embedding(self, feat):
        """Applica spatial_pool (identico a tutto10.py) alla feature map
        spaziale più profonda per ottenere l'embedding EMBED_DIM-dimensionale."""
        return self.spatial_pool(feat)

    def forward(self, x):
        """Ritorna (logits di segmentazione, embedding, bbox predetto
        normalizzato [0,1]). Il decoder U-Net usa le skip connection per una
        segmentazione più precisa; spatial_pool + bbox_head lavorano solo
        sull'ultima feature map (invariato, per compatibilità con tutto10.py)."""
        x0, x1, x2, x3, x4 = self.forward_backbone_features(x)
        ...
        d = self.up4(d, x0)
        d = self.up5(d, None)
        logits = self.seg_head(d)
        if logits.shape[-2:] != x.shape[-2:]:
            logits = F.interpolate(logits, size=x.shape[-2:], mode="bilinear", align_corners=False)

        embed = self.forward_embedding(x4)
...
def run_test(args, device, test_ds):
    model = ResNet18SegModel(init_weights=None).to(device)
...
def train(args, device, train_ds, val_ds):
    ...
    model = ResNet18SegModel(init_weights="imagenet").to(device)
