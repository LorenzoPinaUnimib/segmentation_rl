...
class BatchedActiveLocalizationEnv:
    ...
    def step(self, actions):
        ...
        iou_variation = new_ious - self.previous_ious
        rewards = ciou_variation * 5.0

        # Penalità per ogni step
        rewards -= 0.02

        terminated = (actions == N_ACTIONS - 1)  # azione di trigger = ultima azione (12)
        truncated = (self.current_steps >= MAX_STEPS_PER_EPISODE) & (~terminated)

        task_bonus = torch.zeros_like(rewards)

        term_bonus_all = 10 * (new_ious  - self.tau_iou) #- P_REGRET * local_regret_term
        trunc_bonus_all = term_bonus_all - 2
        task_bonus = torch.where(terminated, term_bonus_all,
                                  torch.where(truncated, trunc_bonus_all, task_bonus))
...
def validate(env, policy_net, val_indices, device, writer, epoch, n_epochs, tag="Validation"):
    ...
    success_rate = (final_ious >= 0.6).float().mean().item()
...
def train(args, device, train_ds, val_ds):
    ...
    def get_epsilon(epoch, max_epoch):
        # Calcola la discesa lineare
        epsilon = EPSILON_START - epoch * (EPSILON_START - EPSILON_END) / (max_epoch * 0.3)
        # Forza il valore a non scendere mai sotto "end"
        return max(EPSILON_END, epsilon)
    
    def get_teacher_prob(epoch, max_epoch):
        # Decadimento esponenziale più veloce
        return max(args.teacher_prob_floor, EPSILON_START - epoch * (EPSILON_START - EPSILON_END) / (max_epoch * 0.5))
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        if epoch_final_ious:
            ...
            success_rate = (np.array(epoch_final_ious) >= 0.6).mean()
        else:
        ...
        print(f"  Success Rate (IoU ≥ 0.6): {success_rate:.4f}")
...
def run_test(args, device, test_ds):
    ...
    success_rate = np.mean(np.array(final_ious_all) >= 0.6)
    ...
    print(f"[INFO] Success rate (IoU ≥ 0.60): {success_rate:.4f} ({int(success_rate*n_test)}/{n_test})")
