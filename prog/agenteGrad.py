...
# spatial_bias_features più sotto)]. BOX_FEAT_DIM è quello che l'ambiente produce da solo
# in _get_obs() (x1,y1,x2,y2 normalizzati + frazione di step); SPATIAL_BIAS_DIM sono le
# feature esplicite di "dove sta il contenuto rilevante nel crop" (bias L/R e T/B, baricentro
# dell'attivazione, dispersione) calcolate dalla feature map del backbone PRIMA del pooling
# globale — invece di lasciare che l'agente lo inferisca solo indirettamente dall'embedding
# collassato. Costo quasi zero (nessun parametro allenabile), nessun retraining del backbone.
BOX_FEAT_DIM = 5
...
class BatchedActiveLocalizationEnv:
    ...
    def step(self, actions):
        ...
        self.previous_cious = new_cious
        
        return self._get_obs(), rewards, (terminated | truncated), new_ious, new_cious
...
class PrioritizedReplayMixin:
    ...
    def _per_mark_new(self, idx):
        # Imposto il device corretto
        idx = idx.to(self._priority_device)

        # Imposto la massima priorità per campionare più spesso
        self.priorities[idx] = self.max_priority

    def set_beta(self, beta):
        # Imposto il parametro beta
...
class PolicyNetwork(nn.Module):
    def __init__(self, history_dim, n_actions, pretrained_backbone=None, use_spatial_attention=None):
        ...
        if use_spatial_attention is True:
            ...
            if pretrained_backbone is not None:
                ...
                missing, unexpected = self.backbone.load_state_dict(fixed_state_dict, strict=False)
                # Diagnostica: con architettura identica a quella di pretrain (SpatialAttentionPool,
                # EMBED_DIM, WARP_SIZE importati DA questo stesso file) ci si aspetta missing=[]
                # (a parte eventualmente "fc." che è già filtrato) e unexpected=[]. Se qui compaiono
                # molte chiavi mancanti/inattese, il checkpoint NON è compatibile con questo backbone
                # (es. proviene da una versione precedente con architettura diversa) e i pesi
                # ImageNet di default per quelle chiavi resteranno invariati SENZA errori evidenti:
                # meglio saperlo subito che scoprirlo da un training che non migliora.
                n_loaded = len(fixed_state_dict) - len(unexpected)
                print(f"[INFO] Backbone pretrainato caricato da '{pretrained_backbone}': "
                      f"{n_loaded} tensori caricati, missing={len(missing)}, unexpected={len(unexpected)}")
                if missing:
                    print(f"[WARN] Chiavi mancanti nel backbone (rimaste ai pesi di init): {missing}")
                if unexpected:
                    print(f"[WARN] Chiavi nel checkpoint non usate dal backbone: {unexpected}")
                if len(missing) > 0 or len(unexpected) > 0:
                    print("[WARN] Il checkpoint di pretrain non combacia perfettamente con "
                          "l'architettura del backbone in questo file: verifica che sia stato "
                          "generato da pretrain_backbone.py con 'from tutto13 import "
                          "SpatialAttentionPool, EMBED_DIM, WARP_SIZE' (stesso file di questo script).")

                # Usa SpatialAttentionPool solo se presenti pesi preaddestrati
                if spatial_pool_state_dict is not None:
                    self.spatial_pool = SpatialAttentionPool(in_channels=self._backbone_channels, embed_dim=EMBED_DIM)
                    # strict=True: spatial_pool DEVE combaciare esattamente (stessa classe importata
                    # da questo file in pretrain_backbone.py), altrimenti è meglio fallire rumorosamente
                    # ora piuttosto che allenare RL su una spatial_pool non correttamente inizializzata.
                    ...
                    self.use_spatial_attention = True
                    print("[INFO] SpatialAttentionPool pretrainata caricata e congelata "
                          "(stessa architettura di pretrain_backbone.py).")
                else:
                    self.spatial_pool = nn.AdaptiveAvgPool2d(1)
                    self.use_spatial_attention = False
                    print("[INFO] Nessuna spatial_pool_state_dict nel checkpoint: uso "
                          "AdaptiveAvgPool2d(1) invece di SpatialAttentionPool (per evitare di "
                          "usare un modulo con parametri random che l'RL non potrà mai allenare, "
                          "essendo il backbone sempre congelato in questa versione ridotta).")
            else:
                self.spatial_pool = nn.AdaptiveAvgPool2d(1)
                self.use_spatial_attention = False

        # Congela SEMPRE tutto il backbone (questa è la versione "ridotta": niente fine-tuning
        # parziale del backbone durante l'RL, solo la testa QNetwork viene allenata; vedi anche
        # l'optimizer in train(), che riceve solo policy_net.head.parameters()).
        for p in self.backbone.parameters():
            p.requires_grad_(False)
        self.backbone.eval()
        # La spatial_pool, se pretrainata, è già stata congelata sopra; se è un AdaptiveAvgPool2d
        # non ha parametri quindi non c'è nulla da congelare, ma la mettiamo comunque in eval()
        # per coerenza/robustezza in caso venga in futuro sostituita con un modulo con stato.
        self.spatial_pool.eval()
        ...
        print(f"[INFO] PolicyNetwork pronta: {n_trainable:,} parametri allenabili (head), "
              f"{n_frozen:,} parametri congelati (backbone"
              f"{' + spatial_pool' if self.use_spatial_attention else ''}).")

    def embed_regions(self, regions, return_bias=False):
        ...
        embed = pooled if self.use_spatial_attention else pooled.view(b, self._backbone_channels)
        if return_bias:
            # Feature esplicite di posizione (bias L/R, T/B, baricentro, dispersione),
            # calcolate dalla stessa feat_map PRIMA che spatial_pool la collassi in un
            # unico vettore globale. Nessun parametro allenabile: costo trascurabile.
            bias_feats = spatial_bias_features(feat_map)
            return embed, bias_feats
        return embed

    def extract_global_features(self, images):
        """Estrae la feature map globale dall'intera immagine una sola volta per episodio."""
        feat_flat = self.backbone(images)
        b = feat_flat.shape[0]
        # ResNet18 + Identity flattena l'output, quindi dobbiamo fargli il reshape (es. [B, 512, 7, 7])
        return feat_flat.view(b, self._backbone_channels, self._backbone_spatial, self._backbone_spatial)

    def embed_rois(self, global_features, rois, return_bias=False):
        """Esegue il crop (RoI Align) direttamente SULLA feature map globale!"""
        # spatial_scale=1.0/32.0 mappa le coordinate pixel 224x224 sulla griglia 7x7 del backbone.
        feat_map = roi_align(
            global_features, rois, 
            output_size=(self._backbone_spatial, self._backbone_spatial),
    ...
    def forward(self, rois_or_embeds, histories, extra_feats, global_features=None):
        if global_features is not None:
            # Siamo in fase di rollout nell'ambiente: usiamo le feature map globali
            embeds, bias_feats = self.embed_rois(global_features, rois_or_embeds, return_bias=True)
            extra_feats = torch.cat([extra_feats, bias_feats], dim=1)
        else:
            # Siamo in fase di aggiornamento dal replay buffer (dove i rois_or_embeds sono già gli embedding finiti)
            embeds = rois_or_embeds
...
def validate(env, policy_net, val_indices, device, writer, epoch, n_epochs, tag="Validation"):
    ...
    with torch.no_grad():
        global_features = policy_net.extract_global_features(obs["images"])
    ...
    last_ciou_per_slot = torch.zeros(num_envs, device=device)
    final_ious = torch.zeros(num_envs, device=device)
    final_cious = torch.zeros(num_envs, device=device)
    ...
    writer.add_scalar(f"{tag}/Final_Avg_CIoU", final_avg_ciou, epoch)
    writer.add_scalar(f"{tag}/Final_Std_CIoU", final_std_ciou, epoch)
    writer.add_scalar(f"{tag}/Final_Best_CIoU", final_best_ciou, epoch)
    ...
    return {
        ...
        'avg_ciou': final_avg_ciou,
        'std_ciou': final_std_ciou,
        'best_ciou': final_best_ciou,
...
def train(args, device, train_ds, val_ds):
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        for step in pbar:
            ...
            next_obs, rewards, dones, ious, cious = train_env.step(actions)
            ...
            # Calcola embedding per il buffer (backbone sempre congelato). Calcoliamo anche
            # le feature di bias spaziale (return_bias=True) qui, PRIMA che spatial_pool
            # collassi tutto in un vettore globale: vanno concatenate a extra ORA, perché
            # una volta salvato nel replay buffer solo l'embedding pooled sopravvive (la
            # feat_map spaziale non viene cachata, per restare leggeri/veloci) e in fase di
            # training (vedi policy_net(b_s, b_hist, b_extra) più sotto) non sarà più
            # ricalcolabile.
            with torch.no_grad():
                step_states, step_bias = policy_net.embed_rois(global_features, obs["rois"], return_bias=True)
                step_next_states, next_bias = policy_net.embed_rois(global_features, next_obs["rois"], return_bias=True)
                step_extra_full = torch.cat([obs["extra"], step_bias], dim=1)
                next_extra_full = torch.cat([next_obs["extra"], next_bias], dim=1)

            ready = n_step_acc.push_and_pop(
                step_states, obs["histories"], step_extra_full, actions, scaled_rewards,
                step_next_states, next_obs["histories"], next_extra_full, dones.float(),
            ...
            if newly_done.any():
                ...
                final_ciou_new = cious[done_indices]
                ...
                epoch_final_cious.extend(final_ciou_new.cpu().tolist())
        ...
        if epoch_final_ious:
            ...
            mean_ciou = np.mean(epoch_final_cious)
            std_ciou = np.std(epoch_final_cious)
            max_ciou = np.max(epoch_final_cious)
            ...
            mean_ciou = std_ciou = max_ciou = float('nan')
        ...
        best_ciou_all = compute_ciou_tensor(best_boxes_all, train_env.gt_boxes)
        ...
        mean_best_ciou = best_ciou_all.mean().item()
        std_best_ciou = best_ciou_all.std().item()
        max_best_ciou = best_ciou_all.max().item()
        ...
        writer.add_scalar("Epoch/Train_Final_CIoU_Mean", mean_ciou, epoch)
        writer.add_scalar("Epoch/Train_Final_CIoU_Std", std_ciou, epoch)
        writer.add_scalar("Epoch/Train_Final_CIoU_Max", max_ciou, epoch)
        ...
        writer.add_scalar("Epoch/Train_Best_CIoU_Mean", mean_best_ciou, epoch)
        writer.add_scalar("Epoch/Train_Best_CIoU_Std", std_best_ciou, epoch)
        writer.add_scalar("Epoch/Train_Best_CIoU_Max", max_best_ciou, epoch)
        ...
        print(f"  Final CIoU : {mean_ciou:.4f} ± {std_ciou:.4f} (max {max_ciou:.4f})")
       ...
       # val_indices = np.random.choice(len(val_ds), size=min(args.batch_size, len(val_ds)), replace=False)
        ...
        val_avg_ciou = val_metrics['avg_ciou']
...
# ─────────────────────────────────────────────────────────────────────────────
# 8bis. GRAD-CAM (per capire "cosa vede" l'agente sulla ROI corrente)
# ─────────────────────────────────────────────────────────────────────────────
def compute_gradcam(policy_net, global_features, roi, history, extra_feats, action_idx):
    """Calcola una mappa di salienza stile Grad-CAM sulla feature map 7x7 (pre spatial_pool)
    corrispondente alla ROI corrente, usando come "target" il Q-value dell'azione scelta
    dall'agente in quello step.

    Nota: il backbone è sempre congelato (requires_grad=False sui suoi parametri), quindi non
    possiamo backpropagare nelle sue attivazioni partendo da 'global_features' calcolate sotto
    torch.no_grad(). Rifacciamo perciò solo la parte "leggera" del forward (roi_align + spatial_pool
    + head) partendo da una copia di global_features con requires_grad_(True): è sufficiente,
    perché la feature map che ci interessa (quella su cui l'agente "guarda" nel crop corrente)
    è proprio l'output di roi_align, e non serve rifare il forward della ResNet.
    """
    gf = global_features.detach().clone().requires_grad_(True)
    roi_d = roi.detach()
    history_d = history.detach()
    extra_d = extra_feats.detach()

    with torch.enable_grad():
        feat_map = roi_align(
            gf, roi_d,
            output_size=(policy_net._backbone_spatial, policy_net._backbone_spatial),
            spatial_scale=1.0 / 32.0, sampling_ratio=2, aligned=True
        )
        b = feat_map.shape[0]
        pooled = policy_net.spatial_pool(feat_map)
        embed = pooled if policy_net.use_spatial_attention else pooled.view(b, policy_net._backbone_channels)
        bias_feats = spatial_bias_features(feat_map)
        extra_full = torch.cat([extra_d, bias_feats], dim=1)
        q_values = policy_net.head(embed, history_d, extra_full)
        score = q_values[0, action_idx]

        # Gradiente SOLO rispetto a feat_map: non tocchiamo i .grad dei parametri del modello
        # (niente side-effect su un eventuale training successivo).
        grads = torch.autograd.grad(score, feat_map, retain_graph=False, create_graph=False)[0]

    activations = feat_map.detach()
    weights = grads.mean(dim=(2, 3), keepdim=True)          # [1, C, 1, 1] media spaziale dei gradienti
    cam = F.relu((weights * activations).sum(dim=1))[0]      # [7, 7]
    cam = cam.cpu().numpy()
...
def overlay_gradcam(base_img_uint8, cam, box, img_shape, alpha=0.5):
    """Sovrappone la heatmap Grad-CAM (7x7, [0,1]) SOLO all'interno della box corrente,
    dato che è esattamente la regione da cui la feature map 7x7 è stata estratta (via roi_align).
    base_img_uint8: immagine HxWx3 uint8 su cui disegnare (stessa convenzione colori del resto del file).
    box: (x1, y1, x2, y2) in pixel, coordinate della ROI corrente sull'immagine.
    """
    ...
    heatmap_full = np.zeros((h_img, w_img), dtype=np.float32)
    if x2c > x1c and y2c > y1c:
        cam_resized = cv2.resize(cam, (x2c - x1c, y2c - y1c), interpolation=cv2.INTER_CUBIC)
        heatmap_full[y1c:y2c, x1c:x2c] = np.clip(cam_resized, 0, 1)

    heatmap_uint8 = np.uint8(255 * heatmap_full)
    heatmap_color = cv2.applyColorMap(heatmap_uint8, cv2.COLORMAP_JET)  # BGR, coerente con cv2.rectangle già usato

    overlay = base_img_uint8.copy()
    blended = cv2.addWeighted(base_img_uint8, 1 - alpha, heatmap_color, alpha, 0)
    mask = heatmap_full > 0.05
...
def run_test(args, device, test_ds):
    ...
    output_gradcam_dir = os.path.join(args.output_root, "gradcams")
    ...
    print(f"[INFO] Generazione Grad-CAM (cosa vede l'agente) in: {output_gradcam_dir}")
    ...
    for i in tqdm(range(n_test), desc="Test set"):
        ...
        with torch.no_grad():
            for _ in range(MAX_STEPS_PER_EPISODE):
                ...
                # Grad-CAM: dove guarda l'agente nella ROI corrente per scegliere l'azione presa.
                # Va calcolato con i gradienti abilitati (siamo dentro un "with torch.no_grad()").
                with torch.enable_grad():
                    cam = compute_gradcam(
                        policy_net, global_features, obs["rois"], obs["histories"], obs["extra"],
                        actions[0].item()
                    )
                gradcam_frame = overlay_gradcam(img, cam, (x1, y1, x2, y2), img.shape, alpha=0.5)
                gradcam_frame = cv2.rectangle(gradcam_frame, (int(gx), int(gy)), (int(gx2), int(gy2)),
                                               (0, 255, 255), 2)
                gradcam_frame = cv2.rectangle(gradcam_frame, (int(x1), int(y1)), (int(x2), int(y2)), color, 2)
                cv2.putText(gradcam_frame, f"GradCAM azione:{actions[0].item()} IoU:{final_iou:.3f}",
        ...
        final_cious_all.append(final_ciou)
        best_ious_all.append(best_iou)
        best_cious_all.append(best_ciou)
        ...
        # Creazione GIF Grad-CAM (stesso schema della gif normale, ma con la heatmap sovrapposta)
        if gradcam_frames:
            final_gradcam_frame = gradcam_frames[-1].copy()
            cv2.putText(final_gradcam_frame, f"FINAL ({result_label}) IoU: {final_iou:.3f}", (5, 45),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2)
            gradcam_frames += [final_gradcam_frame] * 6

        gradcam_gif_name = f"test_{i:04d}_gradcam_finalIoU_{final_iou:.3f}_bestIoU_{best_iou_env:.3f}.gif"
        imageio.mimsave(os.path.join(output_gradcam_dir, gradcam_gif_name), gradcam_frames, fps=5)
    ...
    mean_final_ciou = np.mean(final_cious_all)
    std_final_ciou = np.std(final_cious_all)
    max_final_ciou = np.max(final_cious_all)

    mean_best_iou = np.mean(best_ious_all)
    std_best_iou = np.std(best_ious_all)
    max_best_iou = np.max(best_ious_all)
    mean_best_ciou = np.mean(best_cious_all)
    std_best_ciou = np.std(best_cious_all)
    max_best_ciou = np.max(best_cious_all)
    ...
    with open(summary_path, "w", newline="") as f:
        ...
        csv_writer.writerow([
            "image_index", "final_iou", "final_ciou"
            "best_iou", "best_ciou"
        ...
        for idx in range(n_test):
            csv_writer.writerow([
                ...
                f"{final_cious_all[idx]:.4f}",
                f"{best_ious_all[idx]:.4f}",
                f"{best_cious_all[idx]:.4f}",
    ...
    print(f"[INFO] GIF salvate in: {output_gif_dir}")
    print(f"[INFO] Grad-CAM salvate in: {output_gradcam_dir}")
    ...
    print(f"[INFO] Final CIoU : media {mean_final_ciou:.4f} ± {std_final_ciou:.4f}, max {max_final_ciou:.4f}")
    ...
    print(f"[INFO] Best CIoU  : media {mean_best_ciou:.4f} ± {std_best_ciou:.4f}, max {max_best_ciou:.4f}")
...
def set_seed(seed=42):
    ...
    # Eseguo operazioni per ridurre variabilità
...
if __name__ == "__main__":
    ...
    # Dispositivo su cui svolgere calcoli
    parser.add_argument("--device", type=str, default="auto")
    
    # Percorso della backbone già addestrata
    parser.add_argument("--pretrained-backbone", type=str, default=None,
                        help="Path al checkpoint custom del backbone (con spatial_pool opzionale), "
                             "prodotto da pretrain_backbone.py: stessa architettura di questo file "
                             "(SpatialAttentionPool/EMBED_DIM/WARP_SIZE importati direttamente da qui).")

    # Modalità test
    parser.add_argument("--test", action="store_true", help="Esegui test")
    
    # Modello da utilizzare per test o ripresa esecuzione
    ...
    # Definisco la configurazione dell'esecuzione
    cfg = {
        "dataset": {
    ...
    # Verifico la tipologia di esecuzione
    if args.test:
