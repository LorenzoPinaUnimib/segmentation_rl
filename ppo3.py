...
def compute_action_steps(W: int, H: int):
    ...
    FIX 1: oltre allo step "medio" (usato per i movimenti cx/cy e per il
    resize uniforme w+h), calcoliamo step separati per l'asse w e per l'asse
    h, proporzionali rispettivamente a W e H. Su immagini non quadrate (o su
    tumori con aspect ratio marcato) questo evita che il resize per-asse
    erediti una granularita' "sbagliata" presa dalla dimensione media.
    """
...
# FIX 1: spazio azioni ampliato da 13 a 21 azioni discrete per permettere
# all'agente di adattare l'aspect ratio del box (resize separato per asse
# w/h), invece di poterlo solo scalare uniformemente come nella versione
# precedente. Questo rimuoveva un tetto strutturale alla IoU massima
# raggiungibile su tumori non quadrati, indipendente dal training.
#
# Layout (21 azioni):
...
class VisionMetricsCallback(BaseCallback):
    ...
    def __init__(self, val_dataset, verbose=0, save_dir="./ppo_gradcam_outputs"):
        ...
        # FIX 1: servono anche gli step per-asse (w/h) per replicare esattamente
        # la logica di step() nel Grad-CAM, ora che il resize e' separato per asse.
    ...
    def _on_training_start(self) -> None:
        """Crea il layout dei grafici richiesto all'avvio del modello
        (5_stop_rate: tasso di STOP esplicito; 8_iou_gain: guadagno di IoU
        per episodio scorporato dall'effetto del curriculum, vedi step())."""
            ...
            if isinstance(fmt, TensorBoardOutputFormat):
                layout = {
                    "Analisi_Integrazione_RL": {
                        ...
                        "8_IoU_Gain_per_episodio": ["Multiline", ["custom_plots/8_iou_gain"]],
                    }
                }
                fmt.writer.add_custom_scalars(layout)
                print("[TensorBoard] Layout grafici personalizzati registrato con successo!")
                break
    ...
    def _on_step(self) -> bool:
        ...
        if infos is not None and len(infos) > 0:
            ...
            mean_vals, std_vals, final_vals, stop_vals, gain_vals = [], [], [], [], []
            for info in infos:
                ...
                if metrics is not None:
                    ...
                    gain_vals.append(float(metrics["ep_iou_gain"]))
            if mean_vals:
                ...
                # FIX diagnostico: se questo sale nel tempo, la policy sta
                # davvero migliorando il box episodio dopo episodio, anche se
                # ep_iou_mean/ep_iou_final scendono per l'effetto curriculum
                # (inizializzazione via via piu' difficile, vedi step()).
                self.logger.record("custom_plots/8_iou_gain", float(np.mean(gain_vals)))
        return True
    ...
    def generate_and_save_gradcam(self):
        ...
        if activations is not None and gradients is not None:
            ...
            # FIX 1: logica di movimento identica a BrainTumorRL_Env.step(), con
            # le 21 azioni e il resize separato per asse w/h. Deve restare
            # sincronizzata con step() altrimenti il Grad-CAM visualizza un box
            # mosso con una logica diversa da quella realmente usata in training.
...
class CurriculumCallback(BaseCallback):
    """FIX 2: curriculum sull'inizializzazione del box iniziale in reset().

    Comunica il progresso del training (0 -> 1) a tutti gli env di training
    tramite env_method("set_curriculum_progress", p). Usiamo env_method
    (invece di settare direttamente un attributo Python) perche' con
    USE_SUBPROCESS=True gli env vivono in sottoprocessi separati: env_method
    e' il canale che VecEnv fornisce per eseguire una chiamata di metodo in
    ognuno di essi, attraversando correttamente la catena di wrapper
    VecNormalize -> VecMonitor -> SubprocVecEnv/DummyVecEnv (i VecEnvWrapper
    inoltrano env_method al livello sottostante automaticamente).

    p=0 -> box iniziale generato vicino alla GT (task piu' facile a inizio
    training, quando la policy e' ancora quasi casuale).
    p=1 -> box iniziale generato con il range originale, ampio e centrato
    sull'immagine (task pieno, difficolta' originale).
    """
...
class BrainTumorRL_Env(gym.Env):
    ...
    def __init__(self, pytorch_dataset, max_steps=100):
        ...
        # FIX 2: default = 1.0 (nessun curriculum attivo, range di
        # inizializzazione originale). Il valore viene abbassato solo dal
        # CurriculumCallback sugli env di training; eval_env e gli env creati
        # in evaluate_on_test_set non vengono mai toccati dal callback, quindi
        # restano sempre a piena difficolta' (comportamento invariato rispetto
        # alla versione precedente per valutazione/test).
        self.curriculum_progress = 1.0

    def set_curriculum_progress(self, progress: float):
        """FIX 2: chiamato da CurriculumCallback (via env_method) per aggiornare
        il progresso del curriculum usato in reset() per interpolare l'ampiezza
        dell'inizializzazione del box."""
        self.curriculum_progress = float(np.clip(progress, 0.0, 1.0))

    def action_masks(self):
        """FIX 4 (predisposizione): maschera le azioni di movimento/resize che
        sarebbero no-op perche' w/h sono gia' al limite (min 12px o max W/H).
        Non ancora collegata al training loop (vedi nota FIX 4 in fondo al
        file) — pronta per un'eventuale migrazione a MaskablePPO."""
    ...
    def reset(self, seed=None, options=None):
        ...
        # FIX 2: curriculum sull'inizializzazione del box iniziale. Invece di
        # generare sempre un box casuale fin dal primo timestep (task difficile
        # da subito, quando la policy non ha ancora imparato nulla), interpoliamo
        # tra "vicino alla GT" (curriculum_progress=0) e il range originale,
        # ampio e centrato sull'immagine (curriculum_progress=1). Il progresso
        # e' aggiornato da CurriculumCallback durante il training; resta 1.0
        # (range originale) per eval_env e per gli env di evaluate_on_test_set.
        ...
        # centro: da "centro della GT" a "centro immagine" (range originale)
        pos_center_x = (1 - p) * gt_cx + p * img_cx
        pos_center_y = (1 - p) * gt_cy + p * img_cy
        # rumore di posizione: cresce da +-10px (vicino alla GT) a +-30px (originale)
        ...
        # dimensioni: da "vicino alla GT +-15%" a range originale 20-45% dell'immagine
        ...
        # clip di sicurezza (dimensioni minime/massime valide)
        ...
        # FIX diagnostico: salviamo l'IoU di partenza (post-curriculum) cosi'
        # da poter calcolare, a fine episodio, quanto la policy ha MIGLIORATO
        # il box rispetto al proprio punto di partenza (ep_iou_gain), invece
        # di guardare solo l'IoU assoluta finale — che e' confusa dal
        # curriculum: un curriculum_progress crescente rende l'inizializzazione
        # via via piu' difficile, quindi l'IoU assoluta puo' scendere anche se
        # la policy sta migliorando davvero episodio dopo episodio.
    ...
    def step(self, action):
        ...
        # movimento coarse/fine (invariato rispetto alla versione precedente)
        ...
        # FIX 1: resize separato per asse (coarse) — permette di adattare
        # l'aspect ratio del box, impossibile con il resize uniforme precedente
        elif action == 8:  self.w += bs_w
        elif action == 9:  self.w -= bs_w
        elif action == 10: self.h += bs_h
        elif action == 11: self.h -= bs_h
        # FIX 1: resize separato per asse (fine)
        elif action == 12: self.w += ss_w
        elif action == 13: self.w -= ss_w
        elif action == 14: self.h += ss_h
        elif action == 15: self.h -= ss_h
        # FIX 1: resize uniforme (w e h insieme), mantenuto per scalare in
        # fretta quando l'aspect ratio e' gia' corretto
        elif action == 16:
            self.w += bs; self.h += bs
        elif action == 17:
            self.w -= bs; self.h -= bs
        ...
        if terminated or truncated:
            info["episode_metrics"] = {
                ...
                # FIX diagnostico: quanto la policy ha migliorato il box
                # rispetto al proprio punto di partenza nell'episodio, invece
                # dell'IoU assoluta (confusa dal curriculum). Se questo valore
                # cresce nel tempo, la policy sta davvero imparando, anche se
                # ep_iou_mean/ep_iou_final scendono per l'aumento di difficolta'
                # dell'inizializzazione dovuto al curriculum.
                "ep_iou_gain": float(iou - self.initial_iou),
                "curriculum_progress_at_reset": float(self.curriculum_progress),
            }
...
if __name__ == "__main__":
    # FIX 3: budget di training configurabile senza dover editare il file, cosi'
    # e' facile rilanciare con budget diversi (es. un run breve di verifica
    # rispetto al run lungo verso IoU ~0.7-0.8). Priorita': argomento CLI >
    # variabile d'ambiente TOTAL_TIMESTEPS > default (6M, era 2M).
    arg_parser = argparse.ArgumentParser(description="Training PPO per localizzazione tumori cerebrali via RL")
    ...
    # FIX 5: iperparametri PPO piu' sensibili, isolati qui per poterli iterare
    # a mano senza cercare nel resto del file.
    #
    # Aggiornamento post-run PPO_19: la run precedente (N_EPOCHS=10,
    # clip_range=(0.2,0.1), lr=(3e-4,1e-5)) aveva approx_kl~0.15 e
    # clip_fraction~0.7 (divergenza). Dopo aver stretto manualmente epoche/
    # clip/lr (run PPO_19), approx_kl e' sceso a ~0.003 (ora abbondantemente
    # sotto la soglia 0.01-0.03) e clip_fraction e' tornato in range sano
    # (0.09-0.20, ma in leggera risalita a fine grafico: da ricontrollare).
    # I valori sotto consolidano quella correzione e aggiungono target_kl
    # come rete di sicurezza nativa SB3 (interrompe le epoch di update
    # quando il KL supera la soglia, indipendentemente da n_epochs).
    #   - N_ENVS: numero di env paralleli. Su Windows l'avvio di ogni processo
    #     e' lento; se non sei vincolato da questo overhead alza a 8.
    #   - N_EPOCHS: numero di epoche PPO per rollout. Abbassato da 10 a 5:
    #     con approx_kl ora ben sotto soglia c'e' margine per qualche epoca
    #     in piu' se servisse, ma 5 e' un punto di partenza piu' sicuro di 10.
    #     Se approx_kl torna a superare 0.02-0.03, scendi ulteriormente a 3-4.
    #   - TARGET_KL: soglia nativa SB3, interrompe le epoche di update del
    #     rollout corrente non appena il KL la supera. Complementare a
    #     N_EPOCHS (che limita a priori), agisce a runtime.
    #   - VF_COEF: alzato da 0.5 a 1.0. Il run PPO_19 mostra explained_variance
    #     ancora debole (picco ~0.08-0.09, poi in calo): diamo piu' peso
    #     relativo alla loss della value function per spingerne l'apprendimento.
    #   - GAMMA: fattore di sconto. 0.99 e' un buon default per episodi da
    #     100 step con reward denso; valuta 0.95-0.97 come alternativa se
    #     l'agente sembra "miope" rispetto al terminal_bonus o viceversa
    #     troppo poco reattivo al segnale immediato di delta_iou.
    USE_SUBPROCESS = True
    N_ENVS = 4
    N_EPOCHS = 5
    TARGET_KL = 0.03
    ...
    GAMMA = 0.99  # FIX 5: vedi blocco iperparametri sopra per l'alternativa 0.95-0.97
    ...
    # non nel reward. Per IoU ~0.8 aspettati di aver bisogno di diversi
    # milioni di step complessivi: questo e' un punto di partenza, non un
    # valore garantito (TOTAL_TIMESTEPS default ora 6M, era 2M, vedi FIX 3).

    # FIX 2: il curriculum copre le prime CURRICULUM_TIMESTEPS (20% del budget
    # totale) — dopodiche' curriculum_progress satura a 1.0 (range originale).
    CURRICULUM_TIMESTEPS = max(1, int(0.2 * TOTAL_TIMESTEPS))
    ...
    model = PPO(
        ...
        learning_rate=linear_schedule(1e-4, 1e-5),   # FIX: 3e-4 -> 1e-4 iniziale, consolida la stabilita' osservata in PPO_19
        n_steps=N_STEPS,
        batch_size=512,                              # FIX: 256 -> 512, meno gradient step per rollout (16 minibatch invece di 16 con piu' epoche)
        n_epochs=N_EPOCHS,      # FIX 5: isolato in testa a __main__, abbassa a 3-4 se approx_kl torna a superare 0.02-0.03
        target_kl=TARGET_KL,    # FIX: rete di sicurezza nativa SB3 contro la divergenza (vedi run PPO_18 con approx_kl~0.15)
        gamma=GAMMA,           # FIX: 0.995 -> 0.99, piu' adatto a episodi di 100 step con reward denso
        gae_lambda=0.95,
        clip_range=linear_schedule(0.15, 0.03),       # FIX: 0.2->0.1 era troppo largo (clip_fraction~0.7 in PPO_18)
        ent_coef=0.05,          # valore iniziale; annealed a runtime da EntropyScheduleCallback fino a 0.01
        vf_coef=VF_COEF,        # FIX: 0.5 -> 1.0, spinge l'apprendimento della value function (explained_variance ancora debole in PPO_19)
        max_grad_norm=0.5,
        verbose=1,
...
# ─────────────────────────────────────────────────────────────────────────────
# NOTA RIEPILOGATIVA — FIX 4 e possibili prossimi passi (fuori scope in questo giro)
# ─────────────────────────────────────────────────────────────────────────────
# FIX 4 (action masking): BrainTumorRL_Env.action_masks() e' gia' implementato
# e maschera correttamente le azioni di resize gia' al limite (w/h a 12px o a
# W/H), ma NON e' collegato al training loop: il modello resta PPO standard di
# stable-baselines3, non MaskablePPO. Migrare richiederebbe: dipendenza da
# sb3-contrib, sostituire PPO con MaskablePPO, avvolgere ogni env con
# sb3_contrib.common.wrappers.ActionMasker (o passare action_masks_fn), e
# verificare la compatibilita' di EvalCallback/VecNormalize (che qui e' gia'
# stata sistemata con cura per PPO standard) con MaskableEvalCallback. Per non
# rischiare di rompere quella catena di wrapper gia' verificata, la migrazione
# e' stata lasciata come nota: action_masks() e' pronto, va solo collegato.
#
# Miglioramenti validi ma fuori scope in questo giro (richiedono cambi
# architetturali piu' ampi):
#   - Pretraining supervisionato della CNN come warm start (nuovo script di
#     pretraining + caricamento pesi nel features_extractor).
#   - Migrazione a RecurrentPPO (sb3-contrib): utile perche' l'agente ha
#     memoria zero tra uno step e l'altro oltre a cio' che e' visibile nel
#     canale "box" dell'osservazione; richiede LSTM policy e una nuova
#     dipendenza.
#   - Sostituzione del backbone NatureCNN con un encoder piu' profondo o
#     pretrained (es. ResNet troncato): richiede un features_extractor
#     custom e probabilmente un batch size/learning rate ritarati.