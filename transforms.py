...
def white_balance_gray_world(img: np.ndarray) -> np.ndarray:
    """
    Corregge eventuali dominanti di colore (es. tinte giallastre/verdognole
    introdotte da acquisizione via schermo, compressione JPEG o export da
    scanner diversi) con l'algoritmo "Gray World": assume che, mediata su
    tutta l'immagine, la tinta dovrebbe essere grigio neutro, e riscala ogni
    canale RGB in modo che le loro medie coincidano.

    Su immagini MRI (intrinsecamente in scala di grigi ma spesso salvate/
    esportate come RGB) questo elimina una fonte di variabilita' PURAMENTE
    dovuta al formato del file, non al contenuto anatomico: l'agente/il
    regressore non devono "sprecare" capacita' della CNN per imparare a
    ignorare tinte diverse da immagine a immagine che non portano segnale
    utile per la localizzazione del tumore.

    Non ha effetto (o quasi) se l'immagine e' gia' grigio neutro, quindi e'
    sicuro applicarlo sempre. No-op se l'immagine non e' RGB a 3 canali.
    """
    ...
    if overall_mean < 1e-6:
        return img
    # guadagni limitati per non correggere in modo estremo immagini quasi
    # monocromatiche dove un tumore molto scuro/chiaro sposta la media di un
    # canale (raro ma possibile su tumori molto grandi e molto ipo/iperintensi)
    gains = np.clip(overall_mean / np.clip(means, 1e-6, None), 0.7, 1.4)
    balanced = img_f * gains.reshape(1, 1, 3)
    max_val = 255.0 if img.dtype == np.uint8 else (1.0 if img_f.max() <= 1.0 else float(img_f.max()))
    return np.clip(balanced, 0, max_val).astype(img.dtype)


def apply_clahe(img: np.ndarray, clip_limit: float = 2.0, tile_grid_size: Tuple[int, int] = (8, 8)) -> np.ndarray:
    """
    CLAHE (Contrast Limited Adaptive Histogram Equalization) sul canale di
    luminosita' (spazio LAB), non sui singoli canali RGB direttamente --
    altrimenti si distorce il bilanciamento colore appena corretto da
    white_balance_gray_world. Standard in letteratura per immagini MRI:
    aumenta il contrasto LOCALE (utile per bordi tumore-tessuto poco
    contrastati, la stessa cosa che aiuta sia l'occhio umano che l'agente/
    il regressore a "vedere meglio" il confine) senza schiacciare il range
    dinamico globale come farebbe un'equalizzazione classica sull'intera
    immagine.
    """
    was_gray = (img.ndim == 2)
    img3 = np.stack([img] * 3, axis=-1) if was_gray else img

    if img3.dtype == np.uint8:
        img_u8 = img3
    elif img3.max() <= 1.0:
        img_u8 = (np.clip(img3, 0.0, 1.0) * 255).astype(np.uint8)
    else:
        img_u8 = np.clip(img3, 0, 255).astype(np.uint8)

    lab = cv2.cvtColor(img_u8, cv2.COLOR_RGB2LAB)
    l_ch, a_ch, b_ch = cv2.split(lab)
    clahe = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=tuple(tile_grid_size))
    l_eq = clahe.apply(l_ch)
    out = cv2.cvtColor(cv2.merge([l_eq, a_ch, b_ch]), cv2.COLOR_LAB2RGB)

    if was_gray:
        out = cv2.cvtColor(out, cv2.COLOR_RGB2GRAY)
    if img.dtype == np.uint8:
        return out.astype(np.uint8)
    return (out.astype(np.float32) / 255.0).astype(img.dtype) if img.max() <= 1.0 else out.astype(img.dtype)


def denoise_light(img: np.ndarray, strength: int = 5) -> np.ndarray:
    """
    Denoising leggero (bilateral filter) per attenuare rumore da acquisizione/
    compressione senza sbavare i contorni del tumore -- a differenza di un
    blur gaussiano semplice, il bilateral filter preserva i bordi ad alto
    gradiente (che sono esattamente il segnale che l'agente deve seguire).
    Disattivato di default in config: attivarlo solo se il dataset specifico
    e' visibilmente rumoroso, altrimenti rischia di ammorbidire tumori molto
    piccoli senza benefici concreti.
    """
    if img.dtype == np.uint8:
        img_u8 = img
    elif img.max() <= 1.0:
    ...
    out = cv2.bilateralFilter(img_u8, d=5, sigmaColor=strength * 10, sigmaSpace=strength * 2)
