...
from sb3_contrib.common.maskable.policies import MaskableActorCriticCnnPolicy  # noqa: F401
...
TERMINAL_SCALE_STOP = 18.0     
TERMINAL_SCALE_TIMEOUT = 12.0  

IOU_MILESTONES = [(0.5, 2.0), (0.7, 4.0), (0.8, 8.0)]

# FIX 10: Aumentato il tempo obbligatorio in cui l'agente DEVE esplorare
MIN_STEPS_BEFORE_STOP = 30  
...
class VisionMetricsCallback(BaseCallback):
    ...
    def _compute_mask_for_fixed_box(self):
        ...
        mask[STOP_ACTION] = False # Mai fare stop durante il monitoraggio visivo
        return mask
...
class BrainTumorRL_Env(gym.Env):
    ...
    def action_masks(self):
        ...
        # FIX 10: Rendiamo l'azione STOP fisicamente impossibile da selezionare
        # prima di aver fatto abbastanza esplorazione (MIN_STEPS_BEFORE_STOP).
        if hasattr(self, 'current_step') and self.current_step < MIN_STEPS_BEFORE_STOP:
    ...
    def step(self, action):
        ...
        # FIX 11: Condizioniamo il terminal bonus. Riceve il moltiplicatore vantaggioso 
        # (TERMINAL_SCALE_STOP) SOLO se l'IoU è abbastanza buono da giustificare l'essersi fermati.
        if terminated or truncated:
            if terminated and iou >= 0.4:
                scale = TERMINAL_SCALE_STOP
            else:
                # Se va in timeout, o se l'agente "bara" premendo STOP su un box pessimo, diamo la reward minore.
                scale = TERMINAL_SCALE_TIMEOUT
...
def evaluate_on_test_set(model, test_ds, output_dir, max_steps=100, seed=42, max_samples=0):
    ...
    with open(summary_path, "w") as f:
        ...
        log(f"Tasso STOP esplicito: {stop_rate:.2%}")
        log("─" * 60)
...
if __name__ == "__main__":
    ...
    # Rileggermente rialzata l'entropia per incentivare l'esplorazione visto che non c'è più l'abuso dello STOP.
    entropy_callback = EntropyScheduleCallback(initial_ent=0.03, final_ent=0.005, total_timesteps=TOTAL_TIMESTEPS)
