...
- Funzione di reward $R$: combina una componente di miglioramento incrementale (basata su $\Delta CIoU$), una penalità di step e un bonus / penalità di terminazione (basato su $\Delta IoU$).
- Fattore di sconto $\gamma = 0.90$.
...
Schema a blocchi della pipeline di osservazione:
...
La backbone può essere inizializzata con i pesi pre-addestrati su ImageNet oppure con un checkpoint pre-addestrato specificamente sul dominio (tramite uno script di pre-addestramento).
...
Diagramma dell'architettura completa:
...
Il sistema calcola, ad ogni step, due metriche di sovrapposizione tra la box predetta $B$ e quella di ground truth $B^{gt}$, usate rispettivamente per la valutazione e il reward shaping.
...
La CIoU combina quindi tre criteri di sovrapposizione (area, distanza dei centri, similarità di forma) in un unico segnale continuo e differenziabile, risultando la metrica più informativa anche quando l'intersezione è nulla: per questo è stata scelta come base del reward shaping.
...
Riassumento questa è la nostra reward:
...
L'addestramento è monitorato tramite TensorBoard, con log per epoca (IoU / CIoU correnti, $\varepsilon$, probabilità di guida dell'oracolo, deviazione standard del reward scaler, metriche finali e "migliori durante l'episodio" di training e validation, loss media, success rate).
