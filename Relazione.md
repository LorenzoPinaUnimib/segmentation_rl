...
Pina Lorenzo, 894396 — Rancati Simone, 900052
...
Il progetto affronta il problema della localizzazione di tumori cerebrali in immagini di risonanza magnetica (MRI) formulandolo come un problema di decisione sequenziale: un agente osserva un'immagine e una bounding box corrente e, ad ogni passo, sceglie fra 9 azioni discrete per raffinare tale box fino a farla coincidere con la regione occupata dal tumore. Il sistema combina un backbone convoluzionale pre-addestrato e congelato (ResNet18) con una testa decisionale Dueling Double DQN, addestrata mediante una combinazione di Reinforcement Learning puro, Curriculum Learning sulla soglia di successo e Imitation Learning guidato da un oracolo con ricerca ad albero (lookahead planning). Il reward è costruito a partire dalla metrica Complete-IoU (CIoU), che fornisce segnale anche in assenza di sovrapposizione. L'addestramento è ulteriormente stabilizzato da Prioritized Experience Replay, ritorni n-step, margin loss in stile DQfD e reward scaling adattivo. Il sistema è valutato in termini di IoU, GIoU, DIoU finali e "migliori durante l'episodio", oltre che di reward cumulata e success rate.
...
La localizzazione automatica di lesioni tumorali in immagini MRI è un passo preliminare importante per la diagnosi assistita e per la successiva segmentazione fine. Gli approcci classici basati su object detection (es. reti a regressione diretta delle coordinate, o a proposte di regione) richiedono tipicamente grandi quantità di dati annotati e non modellano esplicitamente il processo *incrementale* con cui un radiologo affina progressivamente l'attenzione su una regione sospetta. Un filone di letteratura alternativo (si veda la Sezione 2) propone di trattare la localizzazione come un problema di **decisione sequenziale**: un agente osserva lo stato corrente (immagine + box) e impara, tramite tentativi ed errori, una politica di trasformazioni elementari della box che la avvicinano iterativamente alla regione target. Questo paradigma è particolarmente interessante in ambito medicale, dove i dataset sono spesso di dimensioni limitate, poiché consente di sfruttare segnali di reward densi (shaping) e dimostrazioni di un esperto (imitation learning) per accelerare l'apprendimento.

### 1.2 Obiettivi e contributo

Il progetto si pone l'obiettivo di progettare, implementare e valutare un agente basato su **Deep Q-Network** in grado di localizzare tumori cerebrali su MRI 2D, esplorando ed integrando congiuntamente:

- una rappresentazione dello stato efficiente, basata su **RoI Align** applicata una sola volta per episodio sulla feature map globale dell'immagine, invece di un forward pass del backbone ad ogni step;
- un meccanismo di **attenzione spaziale** appreso alternativo al semplice average pooling, e feature di bias spaziale a costo computazionale nullo estratte prima del pooling;
- una testa decisionale **Dueling** con aggiornamento **Double DQN**;
- un reward shaping basato sulla metrica **CIoU**, più informativa della IoU pura quando le box non si sovrappongono;
- un **oracolo** che pianifica localmente con una ricerca ad albero (lookahead) sulle azioni disponibili, utilizzato sia per il curriculum di imitation learning sia come sorgente di dimostrazioni per una margin loss in stile **DQfD**;
- meccanismi di stabilizzazione dell'apprendimento (Prioritized Experience Replay, ritorni n-step, reward scaling, target network con soft update).

Il contributo del lavoro è quindi principalmente **sistemistico/ingegneristico**: l'integrazione end-to-end di più tecniche note della letteratura RL in un'unica pipeline applicata a un dominio medicale, con l'obiettivo di valutarne l'effetto complessivo su un compito di localizzazione con dataset di dimensioni contenute.
...
|---|---|---|
| Training | 1501–1502 | ≈ 70% |
| Validation | 429 | ≈ 20% |
| Test | 215 | ≈ 10% |

Ogni immagine è associata a una maschera di segmentazione binaria, da cui viene derivata la bounding box di ground truth calcolando il rettangolo minimo che racchiude tutti i pixel positivi della maschera (se la maschera è vuota viene usata come fallback una box centrata pari a metà dell'immagine).

### 3.1 Preprocessing

Le immagini vengono ridimensionate a una risoluzione fissa di **224×224×3** pixel, coerente con l'input atteso dalla backbone ResNet18. La normalizzazione è eseguita **per immagine** (min-max o standardizzazione calcolata sui singoli campioni) e la maschera viene binarizzata con soglia 0.5. Tecniche opzionali di preprocessing dell'immagine (bilanciamento del bianco, CLAHE, denoising) sono previste nella configurazione ma disattivate nella versione utilizzata per l'addestramento riportato in questa relazione, per mantenere il confronto più semplice da interpretare.

> 🔲 **Placeholder — Figura 1.** Esempi di immagini MRI con box di ground truth sovrapposta (2–3 esempi rappresentativi, incluso un caso con tumore piccolo/periferico e uno con tumore grande/centrale).
...
- **Spazio degli stati $\mathcal{S}$**: l'osservazione a ogni istante $t$ è composta dall'immagine, dalla regione di interesse attorno alla box corrente (con contesto), dalla storia delle ultime azioni e da un vettore di feature ausiliarie (coordinate normalizzate della box, frazione di step trascorsi, feature di bias spaziale). Il dettaglio è discusso nella Sezione 5.
- **Spazio delle azioni $\mathcal{A}$**: 9 azioni discrete che modificano la box (traslazioni, ridimensionamenti, stop). Vedi Sezione 7.
...
- **Funzione di reward $R$**: definita in Sezione 9, combina una componente di miglioramento incrementale (basata su $\Delta$CIoU e $\Delta$IoU), una penalità di step e un bonus/penalità di terminazione.
- **Fattore di sconto $\gamma = 0.98$**.

L'episodio inizia con la box impostata pari all'intera immagine ($x_1=y_1=0$, $x_2=W$, $y_2=H$), fornendo un punto di partenza "estremo" da cui l'agente deve progressivamente restringere l'attenzione. L'episodio termina quando l'agente seleziona l'azione di **stop** (terminazione) oppure quando viene raggiunto un numero massimo di 50 passi senza stop (troncamento). L'ambiente è implementato in forma **vettorizzata/batched**, simulando in parallelo un intero batch di episodi indipendenti sulla stessa GPU/dispositivo, per efficienza computazionale.
...
1. **Embedding visivo della regione corrente.** Anziché ricalcolare l'intera CNN sulla porzione di immagine ritagliata ad ogni step (costoso), la feature map globale dell'immagine viene calcolata **una sola volta per episodio** e, ad ogni step, la regione corrispondente alla box corrente (allargata di un margine di contesto di 16 pixel su ciascun lato) viene estratta direttamente dalla feature map tramite **RoI Align** (che mappa le coordinate in pixel alla griglia 7×7 prodotta dal backbone). Questo riduce drasticamente il costo computazionale per episodio rispetto a un forward completo del backbone ad ogni passo.
2. **Storia delle azioni.** Le ultime 10 azioni sono mantenute come vettore one-hot concatenato (dimensione $10 \times 9 = 90$), aggiornato con una coda FIFO ad ogni step, per dare all'agente memoria del proprio comportamento recente.
...
Come estrattore di feature viene impiegata una **ResNet18**, con lo strato di pooling globale e il classificatore finale rimossi (sostituiti da uno strato identità), mantenendo esclusivamente la porzione convoluzionale. La backbone può essere inizializzata con i pesi pre-addestrati su ImageNet oppure con un checkpoint custom pre-addestrato specificamente sul dominio (tramite uno script di pre-addestramento esterno). In questa versione del sistema la backbone è **sempre interamente congelata**: solo la testa decisionale viene aggiornata durante l'RL, per limitare il numero di parametri allenabili e la possibilità di overfitting dato il dataset di dimensioni contenute.

### 6.2 Pooling spaziale: attenzione vs. media

La feature map prodotta dalla backbone (canali $512 \times 7 \times 7$ per un crop $224\times224$) viene ridotta a un vettore mediante uno di due meccanismi alternativi, selezionabile in base alla disponibilità di un checkpoint pre-addestrato compatibile:

- **Pooling per media**: media semplice delle attivazioni su tutte le posizioni spaziali (baseline).
- **SpatialAttentionPool**: un modulo che raffina la feature map con una convoluzione $3\times3$ + BatchNorm + ReLU, calcola una mappa di attenzione tramite convoluzione $1\times1$ seguita da softmax spaziale, produce un vettore pesato per attenzione che viene concatenato al vettore mediato classicamente, e infine proietta la concatenazione tramite un layer lineare + ReLU nello spazio di embedding finale (di dimensione 512). L'idea è combinare un riassunto "generico" (media) con uno "selettivo" (attenzione), lasciando alla rete la possibilità di enfatizzare le regioni più informative del crop.

### 6.3 Testa decisionale: Dueling DQN

La rete decisionale (QNetwork) riceve in input la concatenazione di embedding visivo, storia delle azioni e feature di coordinate/bias spaziale, e la elabora con un tronco condiviso (due blocchi Linear + LayerNorm + ReLU, dimensione nascosta 512), da cui si diramano due teste distinte in stile **Dueling Network** [5]:
...
Questa separazione consente alla rete di apprendere il valore dello stato anche in situazioni in cui la scelta dell'azione specifica ha scarso impatto (es. box già molto vicina al target), migliorando tipicamente stabilità e velocità di apprendimento rispetto a un DQN "piatto".

### 6.4 Target network e Double DQN

Per stabilizzare il bootstrap del target TD viene mantenuta una **rete target**, copia della rete di policy, aggiornata non tramite backpropagation ma con **soft update (Polyak averaging)** ad ogni step di ottimizzazione:

$$
\theta_{\text{target}} \leftarrow (1-\tau)\,\theta_{\text{target}} + \tau\,\theta_{\text{policy}}
$$

Per ridurre la sovrastima sistematica tipica del Q-learning standard, la selezione dell'azione migliore per lo stato successivo avviene con la rete di **policy** (in modalità di valutazione), mentre la sua valutazione (il relativo Q-value) è ottenuta dalla rete **target**, secondo lo schema **Double DQN** [6]:
...
Il sistema calcola, ad ogni step, quattro metriche di sovrapposizione tra la box predetta $B$ e quella di ground truth $B^{gt}$, usate rispettivamente per il reward shaping (CIoU) e per la valutazione (IoU, GIoU, DIoU, CIoU).
...
Limite noto della IoU: quando le due box non si intersecano, IoU $= 0$ indipendentemente da quanto siano vicine o lontane, fornendo un gradiente nullo/non informativo.
...
dove $C$ è il più piccolo rettangolo che racchiude sia $B$ che $B^{gt}$. La GIoU introduce un termine di penalità basato sull'area "sprecata" del box che racchiude entrambe le box, restando informativa anche a intersezione nulla [11].
...
dove $\rho(\cdot)$ è la distanza euclidea tra i centri delle due box e $c$ è la diagonale del più piccolo rettangolo che le racchiude entrambe. Penalizza direttamente la distanza tra i centri, favorendo una convergenza più diretta rispetto a GIoU [10].
...
La CIoU combina quindi tre criteri di sovrapposizione (area, distanza dei centri, similarità di forma) in un unico segnale continuo e differenziabile, risultando la metrica più informativa tra quelle implementate anche quando l'intersezione è nulla: per questo è stata scelta come base del reward shaping (Sezione 9), mentre IoU/GIoU/DIoU sono mantenute come metriche di valutazione a fine episodio.

> 🔲 **Placeholder — Figura 4.** Confronto grafico tra IoU, GIoU, DIoU e CIoU al variare della distanza tra due box non sovrapposte (per illustrare perché la sola IoU è meno informativa).
...
Ad ogni step viene premiata la variazione di CIoU e, in misura minore, di IoU tra lo stato corrente e quello precedente:

$$
\Delta\text{CIoU}_t = \text{CIoU}_t - \text{CIoU}_{t-1}, \qquad \Delta\text{IoU}_t = \text{IoU}_t - \text{IoU}_{t-1}
$$

$$
r^{\text{move}}_t = 5 \cdot \Delta\text{CIoU}_t + 2 \cdot \Delta\text{IoU}_t
$$

La componente dominante è quella basata su CIoU (peso 5), scelta per la sua maggiore informatività; la componente su IoU pura (peso 2) rinforza direttamente la metrica usata anche per il criterio di successo.
...
L'episodio termina quando l'agente seleziona l'azione di stop (indice 8). Alla terminazione viene aggiunto un bonus proporzionale a quanto la IoU finale supera (o non raggiunge) la soglia di successo fissa $0.6$:

$$
r^{\text{term}} = 10 \cdot (\text{IoU}_T - 0.6)
$$
...
Questa struttura incentiva l'agente a fermarsi quando la sovrapposizione è già buona (bonus positivo crescente con la IoU) e lo penalizza sia per uno stop prematuro con IoU bassa, sia — più severamente — per il mancato raggiungimento di una decisione entro il budget di step disponibile.

### 9.4 Clipping e scaling

Il reward totale per lo step è la somma delle componenti precedenti (con il bonus di terminazione/troncamento che sostituisce, non si somma al termine di step quando presente) ed è infine limitato nell'intervallo $[-10, 10]$ per evitare aggiornamenti instabili dovuti a valori anomali. Prima di essere inserito nel replay buffer, il reward viene inoltre normalizzato in ampiezza (non ricentrato) da uno **scaler a media/varianza mobile** (aggiornato online), dividendo per la deviazione standard corrente stimata sui reward osservati — una forma di *reward scaling* che stabilizza la scala dei target TD indipendentemente dal punto dell'addestramento.

> 🔲 **Placeholder — Tabella 2.** Riepilogo delle componenti di reward con formula, peso e condizione di applicazione (tabella riassuntiva delle sottosezioni 9.1–9.3).
...
Il replay buffer (EmbeddingReplayBuffer) memorizza, per ciascuna transizione, l'embedding visivo (anziché l'immagine grezza, per risparmiare memoria), la storia delle azioni, le feature extra, azione, reward, flag di terminazione e un'etichetta di provenienza esperta (per distinguere le transizioni suggerite dall'oracolo). Il campionamento non è uniforme ma **prioritizzato** [7]: la probabilità di estrarre una transizione è proporzionale al suo errore TD assoluto elevato a un esponente $\alpha$ (pari a 0.6):
...
Anziché un bootstrap a singolo step, le transizioni sono accumulate in code per-episodio e combinate in ritorni **n-step** (con un orizzonte di 50 passi, effettivamente pari al limite massimo di step per episodio in questa configurazione):
...
Configurazione di base: l'agente apprende esclusivamente dall'interazione con l'ambiente e dal segnale di reward, senza guida esterna, selezionando le azioni con una politica **$\varepsilon$-greedy** rispetto ai Q-value correnti. $\varepsilon$ decresce linearmente da 1.0 a 0.05 nella prima metà del training.
